"""
dirt_sniffer: Particle Detection Gallery
Simple version - upload images, detect, view/edit in gallery
All features: filters, delete, mass edit, zoom/pan, undo, export
"""
...
st.set_page_config(page_title="dirt_sniffer", page_icon="🧹", layout="wide")
...
with st.sidebar:
    ...
    files = st.file_uploader("Images (JPG, PNG, TIFF)", type=["jpg", "png", "tif", "tiff"], accept_multiple_files=True)
                    ...
                    try:
                        with tempfile.NamedTemporaryFile(delete=False) as tmp:
                            ...
                            if particles:
                                ...
                                status.text(f"⚠️ {f.name}: No particles found")
                    except Exception as e:
                        status.text(f"❌ {f.name}: Error")
    ...
    if st.button("📥 Export CSV"):
        ...
        if rows:
            ...
            st.download_button("⬇️ Download", csv, f"results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                               "text/csv")
